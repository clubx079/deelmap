...
export default function PropertyCard({ property, isLoggedIn = false, layout = 'horizontal' }) {
  ...
  const shareUrl = `https://deelmap.com/${slug}`
    ...
    <>
      ...
      {layout === 'vertical' ? (
        ...
        <div
          style={is_highlighted ? { padding: '2px', background: 'linear-gradient(135deg, #D03839 0%, #E8633E 50%, #D03839 100%)', borderRadius: '7px', animation: 'pulse-glow 3s ease-in-out infinite' } : {}}
        >
          <div className={`bg-white ${is_highlighted ? '' : 'border border-[#E8E8E4]'} rounded overflow-hidden hover:shadow-md transition-shadow duration-200 flex min-h-[230px]`} style={is_highlighted ? { borderRadius: '5px' } : {}}>
            {/* Photo */}
            <Link href={`/${slug}`} className="relative flex-shrink-0 w-[280px] self-stretch block group">
              {is_highlighted && (
                <div className="absolute top-0 inset-x-0 h-[3px] z-10" style={{ background: 'linear-gradient(90deg, #D03839, #E8633E, #D03839)' }} />
              )}
              {thumbnailImage ? (
