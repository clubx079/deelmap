...
export function MobileNav({ isOpen, onClose, menuItems, user, onShowAuth, onLogout }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] md:hidden">
      ...
      {/* Sidebar */}
      <div className="fixed right-0 top-0 h-full w-80 bg-[#1A1816] transform transition-transform duration-300 ease-in-out shadow-2xl z-[10000]">
        {/* Header */}
            ...
            <div key={item.label} className="relative">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {item.hasNew && (
                    <span className="bg-[#D03839] text-white text-xs px-2 py-1 rounded-full font-semibold">
                      NEW
                    </span>
                  )}
                </div>
        ...
        {/* User Section */}
        <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-gray-600 bg-[#1A1816]">
          {user ? (
            <div className="space-y-4">
              <div className="flex items-center space-x-3 text-white">
                <div className="w-10 h-10 bg-[#D03839] rounded-full flex items-center justify-center">
