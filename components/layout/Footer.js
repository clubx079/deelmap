...
function AccordionSection({ title, links, isOpen, onToggle }) {
  return (
    <div className="border-b border-[#2A2824]">
      ...
      >
        <span className="text-[13px] font-semibold text-white uppercase tracking-[1.5px]">
          {title}
        </span>
        <ChevronDown
          className={`w-4 h-4 text-[#737370] transition-transform duration-200 flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      <div
        ...
        <ul className="space-y-3 pl-1">
          {links.map(l => (
            <li key={l.label}>
              <Link
                href={l.href}
                className="text-[14px] text-[#737370] hover:text-white transition-colors"
              >
                {l.label}
              </Link>
            </li>
  ...
  return (
    <footer>
      ...
      <div className="bg-[#1A1816] relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-12 lg:px-[100px] py-14 lg:py-20">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 lg:gap-10">
            ...
            <div className="max-w-lg">
              ...
              <h2 className="text-[32px] sm:text-[40px] lg:text-[48px] font-bold text-white leading-tight mb-4">
                Find and close better{' '}
            ...
            <div className="flex flex-col gap-3 lg:min-w-[260px]">
              <Link
                href="/marketplace"
                className="flex items-center justify-center gap-2 h-13 py-3.5 bg-[#D03839] hover:bg-[#E0493B] active:bg-[#C73022] text-white font-semibold text-[16px] rounded transition-colors"
              >
                Start finding deals →
              </Link>
              <Link
                href="/join-seller"
                className="flex items-center justify-center py-3.5 border border-[#444441] text-white hover:border-[#737370] font-medium text-[15px] rounded transition-colors"
              >
                List a property
              </Link>
      ...
      <div className="bg-[#1A1816]">
        <div className="w-full px-6 md:px-12 lg:px-[100px] py-10 lg:py-14">
          ...
          <div className="lg:hidden">
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-[#737370] leading-relaxed mb-5">
                The trusted marketplace for verified off-market real estate deals across the United States.
              </p>
              {/* Social Icons */}
              <div className="flex items-center gap-2">
                <a href="https://twitter.com" target="_blank" rel="noopener noreferrer"
                  className="w-9 h-9 border border-[#2A2824] rounded flex items-center justify-center text-[#737370] hover:text-white hover:border-[#737370] transition-colors">
          ...
          <div className="hidden lg:grid lg:grid-cols-7 gap-10">
            {/* Brand */}
            ...
            {SECTIONS.map(section => (
              <div key={section.title}>
                <h4 className="text-[11px] font-semibold text-white uppercase tracking-[1.5px] mb-5">{section.title}</h4>
        ...
        {/* Bottom bar */}
        <div className="border-t border-[#2A2824]">
          <div className="w-full px-6 md:px-12 lg:px-[100px] py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-[13px] text-[#737370]">© 2026 DeelMap. All rights reserved.</p>
            <div className="flex items-center gap-5">
              <Link href="/privacy-policy" className="text-[13px] text-[#737370] hover:text-white transition-colors">Privacy</Link>
              <Link href="/terms-of-use" className="text-[13px] text-[#737370] hover:text-white transition-colors">Terms</Link>
              <Link href="/cookie-policy" className="text-[13px] text-[#737370] hover:text-white transition-colors">Cookies</Link>
            </div>
          </div>
        </div>
      </div>
