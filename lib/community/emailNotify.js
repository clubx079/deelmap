...
export function emailNewLotPost({ to, authorHandle, lotName, postTitle, postSlug }) {
  const href = `${APP_URL}/community/p/${postSlug}`
  sendCommunityEmail({
    to,
    subject: `New in ${lotName}: ${postTitle}`,
    html: shell({
      title: postTitle,
      preview: `@${authorHandle} posted in ${lotName}`,
      intro: `@${authorHandle} just posted in ${lotName} — a Lot you follow.`,
      ctaLabel: 'Read the post',
      ctaHref: href,
      footerHint: `You\'re getting this because you subscribed to the ${escape(lotName)} Lot. Manage your subscriptions any time from your community profile.`,
    }),
  })
}
