...
// Single source of truth for a post + its comment tree, used by both the API
// route and the server-rendered page (so SSR content matches the client exactly).
// Wrapped in React cache() so generateMetadata + the page share one fetch per request.
export const getPostBySlug = cache(async (slug, userId = null) => {
  ...
  // Related posts from the same lot
  ...
  // Viewer vote / save state (only when a user is known — SSR passes null)
