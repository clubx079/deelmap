...
async function checkText(text, address) {
  ...
  try {
    ...
    const reply = await groqChat([{
      role: 'user',
      content: `You are reviewing a listing description for a real estate wholesale marketplace.\n\nA description PASSES only if it genuinely describes the property — its condition, location, features, rooms, investment potential, renovation needs, neighborhood, or similar real estate details.\n\nA description FAILS if it:\n- Does not actually describe a property (e.g. "i hate this earth", "test", "hello world", random words)\n- Contains personal rants, complaints, or opinions unrelated to the property\n- Contains profanity, threats, scam language, or sexual content\n- Is too vague or nonsensical to be a real listing\n\nReply with ONLY "PASS" or "FAIL: <brief reason>". Do not explain further.\n\n${addressContext}Description to review:\n${text.slice(0, 1200)}`
