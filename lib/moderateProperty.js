    ...
    try {
      reply = await groqChat([{
        ...
        content: [
          ...
          { type: 'text', text: 'Does this image show the interior or exterior of a real building or property (house, apartment, room, yard, driveway, vacant lot)? Answer YES or NO only. A distressed, run-down, or messy property still counts as YES.' }
        ]
      }], 'meta-llama/llama-4-scout-17b-16e-instruct')
    } catch (apiErr) {
      // Groq API error (rate limit, auth, model issue) — fail-open so users aren't blocked by outages
      console.error('[moderation] Groq vision API error (failing open):', apiErr?.message)
      return { pass: true }
    }

    console.log(`[moderation] Vision reply: "${reply}"`)
    if (reply.toUpperCase().startsWith('YES')) return { pass: true }
    return { pass: false, reason: 'Photo does not appear to show a property. Please upload photos of the building exterior, interior rooms, yard, or lot.' }
  } catch (err) {
