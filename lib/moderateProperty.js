...
async function checkText(text, address) {
  ...
  try {
    ...
    const reply = await groqChat([{
      role: 'user',
      content: `You are moderating a real estate marketplace listing. Reject the text if it contains ANY of the following: profanity, hate speech, scam content, threats, sexual content, random/nonsensical sentences, personal rants, or content that is clearly not a real property description (e.g. "I hate this property", "test test", "asdfgh", off-topic content). A valid listing description must be relevant to the property itself. Reply with exactly "PASS" if it is appropriate property-related content, or "FAIL: <brief reason>" if not.\n\n${addressContext}${text.slice(0, 1200)}`
