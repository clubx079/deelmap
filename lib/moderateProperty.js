...
async function checkImage(url) {
  if (!url) return { pass: true }
  try {
    // Fetch the image ourselves and convert to base64 so Groq can access it
    // regardless of whether the Supabase storage URL requires auth
    const imgRes = await fetch(url)
    if (!imgRes.ok) {
      return { pass: false, reason: 'Image could not be loaded. Please re-upload your photos.' }
    }
    ...
    const reply = await groqChat([{
      ...
      content: [
        ...
        { type: 'text', text: 'You are a strict image moderator for a real estate wholesale marketplace. Your job is to verify that photos uploaded to property listings are genuine property photos.\n\nREPLY "PASS" ONLY if the image clearly and unambiguously shows a physical property — such as a house exterior, interior room, kitchen, bathroom, bedroom, living room, garage, driveway, yard, land, or any part of a building structure.\n\nREPLY "FAIL: <brief reason>" for ANY of the following — no exceptions:\n- You cannot see any image, the image is blank, broken, or inaccessible\n- Receipts, invoices, bills, or any financial documents\n- Text-heavy images, scanned documents, or printed paper of any kind\n- People, faces, or body parts\n- Animals or pets\n- Food or drinks\n- Memes, cartoons, illustrations, or artwork\n- Screenshots of apps, websites, social media, or digital content\n- Random objects, products, or items not physically attached to a building\n- Nature scenery, landscapes, or outdoor scenes with no building visible\n- Blurry, pitch-black, or all-white images with no recognizable content\n- Anything that is not clearly a photo of real estate\n\nIf you are not 100% certain this is a genuine photo of a real property, reply FAIL.\nWhen in doubt, FAIL.' }
      ]
    }], 'llama-3.2-90b-vision-preview')
    if (reply.toUpperCase().startsWith('PASS')) return { pass: true }
    return { pass: false, reason: reply.replace(/^FAIL:\s*/i, '').slice(0, 200) }
  } catch (err) {
    console.error('[moderation] Vision check error:', err?.message)
    return { pass: false, reason: 'Image could not be verified. Please re-upload your photos.' }
  }
}
