...
export function normalizeWholesaleDeal(deal) {
  ...
  return {
    ...
    // Photos - only featured + first few for card display (full set loaded on detail page)
    property_photos: (() => {
      if (!Array.isArray(deal.property_photos)) return [];
      const photos = deal.property_photos.map(p => ({
        photo_url: p.optimized_url || p.photo_url,
        optimized_url: p.optimized_url || null,
        is_featured: p.is_featured || false
      }));
      // Put featured first, then limit to 5 for list view
      const featured = photos.filter(p => p.is_featured);
      const rest = photos.filter(p => !p.is_featured).slice(0, 4);
