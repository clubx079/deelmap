...
export function normalizeWholesaleDeal(deal) {
  ...
  return {
    ...
    // Display-only flag: scraped deal is no longer available (real status is kept above for analytics)
    sold: ['sold', 'expired'].includes(String(deal.status || '').toLowerCase()),

    // Investment metrics (scraped deals often have these)
    arv: deal.arv || null,
    repair_cost: deal.repair_cost || null,
    ...
    // Photos — featured photo for card, full set loaded on detail page
    property_photos: Array.isArray(deal.property_photos)
      ...
      : [],
    photo_count: deal.photo_count || (Array.isArray(deal.property_photos) ? deal.property_photos.length : 0),
    
    // Contact/seller info
    temp_seller_id: deal.temp_seller_id || null,
    seller_id: null,
    seller_name: deal.temp_seller_logins?.seller_name || null,
    is_homepage_featured: false,

    // Listing type + auction-specific fields
    listing_type: deal.listing_type || null,
    auction_date: deal.auction_date || null,
    auction_time: deal.auction_time || null,
    auction_location: deal.auction_location || null,
    listing_url: deal.listing_url || null,
...
export function normalizeManualProperty(property, property_images = []) {
  ...
  return {
    ...
    // Internal truth kept for analytics; `sold` is a display-only flag (real-sold + marketing-sold/expired)
    property_status: property.property_status || null,
    sold: ['sold', 'expired'].includes(String(property.property_status || '').toLowerCase()),
    ...
    seller_name: null,
    is_homepage_featured: property.is_homepage_featured || false,
