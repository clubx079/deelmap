...
export function normalizeWholesaleDeal(deal) {
  ...
  return {
    ...
    // Photos - just featured photo for marketplace card (full set loaded on detail page)
    property_photos: (() => {
      if (!Array.isArray(deal.property_photos) || deal.property_photos.length === 0) return [];
      const featured = deal.property_photos.find(p => p.is_featured);
      const first = deal.property_photos[0];
      const photo = featured || first;
      return [{ photo_url: photo.optimized_url || photo.photo_url, is_featured: true }];
    })(),
    photo_count: Array.isArray(deal.property_photos) ? deal.property_photos.length : 0,
