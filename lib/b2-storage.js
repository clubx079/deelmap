...
//
// deelmap-originated listing photos are uploaded here (same private "Deelmap" bucket
// the DeelScout scraper uses) and SERVED by the DeelScout signing endpoint at
// IMG_PUBLIC_BASE (`<IMG_PUBLIC_BASE>/api/img/<key>`), which mints a short-lived
// presigned B2 GET and 302-redirects. So the URL stored in the DB is that /api/img URL.
//
// Never import this from client components — it holds the B2 secret. Upload happens
// through the /api/upload-listing-image server route.
//
// Env:
...
export async function uploadToB2(buffer, key, contentType = 'image/webp') {
