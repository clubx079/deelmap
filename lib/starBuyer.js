...
// DeelMap is a US marketplace: a star buyer must be CONFIRMED US-based. A missing
// or non-US location disqualifies, no matter how business-like the email looks.
...
export function isUSCountry(country) {
  return US_VALUES.has(String(country || '').trim().toLowerCase())
}
...
export async function classifyBuyer({ name = '', email = '', country = '', statesOfInterest = [] } = {}) {
  // Location gate — fast, no network: a non-US or unknown location can never be a
  // star buyer. Short-circuits BEFORE the Groq call, so most non-US signups skip
  // the LLM entirely and add zero latency.
  if (!isUSCountry(country)) {
    return {
      isStar: false, confidence: 0.9, persona: 'casual', company: '',
      reason: country ? `Non-US location (${country})` : 'No confirmed US location',
      method: 'location', signal: 'none',
    }
  }
