...
export default function MarketInsightsPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-48" style={{ fontFamily: 'var(--font-dm-sans), sans-serif' }}>
  ...
  return (
    <div className="p-4 lg:p-6" style={{ fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold text-[#1A1816] tracking-[-0.44px]">Market Insights</h1>
          <p className="text-[14px] text-[#737370] mt-1">Live data from {summary.total?.toLocaleString() || 0} active wholesale deals</p>
        </div>
        <Link
          href="/marketplace"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {statCards.map((card) => (
          <div key={card.label} className="bg-white border border-[#E8E8E4] rounded p-4">
            <div className="flex items-start justify-between mb-3">
              <p className="text-[12px] font-medium text-[#737370]">{card.label}</p>
              <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: card.iconBg }}>
                {card.icon}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        {/* Top markets */}
        <div className="lg:col-span-1 bg-white border border-[#E8E8E4] rounded overflow-hidden">
