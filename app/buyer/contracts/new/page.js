...
export default function BuyerNewContractWizardPage() {
  ...
  useEffect(() => {
    ...
    setSellerName(user.name || user.full_name || user.first_name || user.email || '')
  }, [user])
  ...
  function canProceed(s) {
    ...
    if (s === 3) return /\S+@\S+\.\S+/.test(buyerEmail)
    if (s === 4) {
      ...
      <div className="p-4 lg:p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#737370]">Review and sign below — the counterparty gets a signing link once you finish.</p>
          </div>
        </div>
  ...
  const STEP_LABELS = ['Contract Type', 'Property', isAssignment ? 'Assignee info' : 'Buyer info', 'Terms', 'Review']
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-[#E8E8E4] rounded p-5">
        {step === 1 && <Step1ContractType templates={templates} templatesLoading={templatesLoading} templateId={templateId} onSelect={setTemplateId} />}
        ...
        {step === 3 && <Step3Counterparty buyerName={buyerName} buyerEmail={buyerEmail} onBuyerNameChange={setBuyerName} onBuyerEmailChange={setBuyerEmail} template={template} coSellerName={fieldValues.co_seller_name} onCoSellerNameChange={v => setFieldValues(prev => ({ ...prev, co_seller_name: v }))} />}
        {step === 4 && <Step4Terms values={fieldValues} onChange={setField} template={template} />}
        {step === 5 && <Step5Review template={template} propertyId={propertyId} property={properties.find(p => p.id === propertyId)} buyerName={buyerName} buyerEmail={buyerEmail} fieldValues={fieldValues} sellerName={sellerName} sellerEmail={sellerEmail} onJump={setStep} />}
      </div>
      ...
      <div className="flex items-center justify-between mt-5">
        ...
        {step < 5 ? (
          ...
          <button onClick={handleSend} disabled={sending || !buyerEmail || !templateId} className="h-10 px-5 flex items-center gap-1.5 text-[13px] font-semibold bg-[#D03839] hover:bg-[#E0493B] text-white rounded disabled:opacity-50 disabled:cursor-not-allowed">
            {sending ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : 'Send Contract'}
          </button>
        )}
      </div>
...
function Step1ContractType({ templates, templatesLoading, templateId, onSelect }) {
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-[#737370] mb-4">Choose the template that matches the deal you're sending.</p>
      <div className="space-y-2">
        {templates.map(t => {
          const selected = String(t.id) === String(templateId)
          return (
            <button key={t.id} type="button" onClick={() => onSelect(String(t.id))} className={`w-full text-left p-4 border rounded transition-colors ${selected ? 'border-[#D03839] bg-[#FEF0EF]/40' : 'border-[#E8E8E4] hover:border-[#1A1816]'}`}>
...
function Step3Counterparty({ buyerName, buyerEmail, onBuyerNameChange, onBuyerEmailChange, template, coSellerName, onCoSellerNameChange }) {
  const isAssignment = template?.slug === 'assignment'
  const partyLabel = isAssignment ? 'Assignee' : 'Buyer'
  const [coSellerOpen, setCoSellerOpen] = useState(!!coSellerName)
  return (
    <div>
      <h2 className="text-[16px] font-bold text-[#1A1816] mb-1">{partyLabel} info</h2>
      <p className="text-[13px] text-[#737370] mb-4">{isAssignment ? "The party you're assigning the contract to. They'll receive a copy to sign once you finish." : "Who's signing on the buyer side?"}</p>
      <div className="space-y-4">
        <div>
          <label className={LABEL_CLS}>{partyLabel} Full Name</label>
          <input type="text" value={buyerName} onChange={e => onBuyerNameChange(e.target.value)} placeholder="John Smith" className={INPUT_CLS} />
        </div>
        <div>
          <label className={LABEL_CLS}>{partyLabel} Email <span className="text-[#D03839]">*</span></label>
          <input type="email" value={buyerEmail} onChange={e => onBuyerEmailChange(e.target.value)} placeholder={`${partyLabel.toLowerCase()}@example.com`} className={INPUT_CLS} required />
          <p className="text-[12px] text-[#A8A8A4] mt-1.5">The {partyLabel.toLowerCase()} will only get a signing link after you sign first.</p>
        </div>
      </div>
      {template?.slug === 'purchase' && (
        <div className="mt-5 pt-5 border-t border-[#E8E8E4]">
          {!coSellerOpen ? (
            <button type="button" onClick={() => setCoSellerOpen(true)} className="inline-flex items-center gap-1 text-[12px] font-semibold text-[#D03839] hover:text-[#B82F30]">+ Add co-seller (optional)</button>
          ) : (
            <div>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-[13px] font-semibold text-[#1A1816]">Co-seller</p>
                  <p className="text-[12px] text-[#737370]">For jointly-owned properties. Their name will print on the contract; they'll sign by hand on the printed copy.</p>
                </div>
                <button type="button" onClick={() => { onCoSellerNameChange(''); setCoSellerOpen(false) }} className="text-[12px] text-[#737370] hover:text-[#1A1816]">Remove</button>
              </div>
              <label className={LABEL_CLS}>Co-seller Full Name</label>
              <input type="text" value={coSellerName} onChange={e => onCoSellerNameChange(e.target.value)} placeholder="Jane Smith" className={INPUT_CLS} />
            </div>
          )}
        </div>
      )}
...
function Step4Terms({ values, onChange, template }) {
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-[#737370] mb-4">{isAssignment ? 'Numbers and dates that pre-fill on the assignment contract. Required fields are marked.' : 'Numbers, dates, and counterparty info that pre-fill on the purchase contract.'}</p>
      ...
      {!isAssignment && (
        <div className="mt-6">
          <p className="text-[11px] font-bold text-[#A8A8A4] uppercase tracking-wide mb-3">Property Details</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {!isAssignment && (
        <div className="mt-6">
          <p className="text-[11px] font-bold text-[#A8A8A4] uppercase tracking-wide mb-3">Parties' Addresses</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FieldRow label="Your Address (Seller)"><input type="text" value={values.seller_address || ''} onChange={e => onChange('seller_address', e.target.value)} placeholder="123 Your St, City, ST 00000" className={INPUT_CLS} /></FieldRow>
            <FieldRow label="Buyer's Address" hint="counterparty"><input type="text" value={values.buyer_address || ''} onChange={e => onChange('buyer_address', e.target.value)} placeholder="Where to send paperwork" className={INPUT_CLS} /></FieldRow>
      ...
      {isAssignment && (
        <div className="mt-6">
          ...
          <p className="text-[12px] text-[#737370] mb-3">Reference the original Purchase &amp; Sale Contract you're assigning. Who the seller is and when you signed it both appear in the Whereas clause.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
function Step5Review({ template, propertyId, property, buyerName, buyerEmail, fieldValues, sellerName, sellerEmail, onJump }) {
  ...
  const rows = [
    { section: 'Contract Type', step: 1, items: [{ label: 'Template', value: template?.label || template?.name || '—' }] },
    { section: 'Property',      step: 2, items: [
    ...
    { section: 'Terms',         step: 4, items: termsItems },
  ]
  return (
    <div>
      <h2 className="text-[16px] font-bold text-[#1A1816] mb-1">Review &amp; send</h2>
      <p className="text-[13px] text-[#737370] mb-4">Double-check everything below. Hit Send Contract and you'll be taken to sign your part — the counterparty gets their link after you finish.</p>
      <div className="space-y-4">
        {rows.map(group => (
